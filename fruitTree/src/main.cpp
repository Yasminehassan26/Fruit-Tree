...
void Tree:: magnifyByType(string type, int value)
{
    ...
    while (currentFruit != NULL || nodes.empty() == false)
    {
        ...
        if(currentFruit ->getType().find(type)!= string::npos)
        {
            ...
            toDelete.push_back(currentFruit);
        }
        nodes.pop();
        currentFruit = currentFruit->getRight();
    }
...
/////////////////////////////////////
void print2D(Fruit* root, int space)
{
    // Base case
    if (root == NULL)
        return;
    ...
    for (int i = COUNT; i < space; i++)
        printf(" ");
    cout<<root->getWeight()<<" "<<root->getSpecific()<<"\n";
 
    // Process left child
    print2D(root->getLeft(), space);
}
void printTree(Fruit* root)
{
   print2D(root, 0);
}
///////////////////////////////////
int main()
{
	...
	cout<<"\n----------------------------------------------------- magnify \n ";
    b.Insert(NULL,root,"Apple", 100);
    b.Insert(NULL,root,"Berry", 110);
    b.Insert(NULL,root,"Strawberry", 70);
    printTree(root);
